...
Vector Lat3DHEX1stO::Gx;
Vector Lat3DHEX1stO::Gt;
Vector Lat3DHEX1stO::Gp;
Vector Lat3DHEX1stO::G2;


Lat3DHEX1stO::Lat3DHEX1stO(Input* MyInput_, Text name_)
...
Lat3DHEX1stO::~Lat3DHEX1stO() {
}

Boolean
Lat3DHEX1stO::OverflowProtection(void) const {
	return false;
}
void
Lat3DHEX1stO::MakeSafe(Vector) const {
}

void
Lat3DHEX1stO::RestoreFromSafe(Vector) const {
}

void
Lat3DHEX1stO::GetLatticeInfo(int* Info) const{ 
	Info[0]=3; //Dimensions
	Info[1]=3; //HEX;
...
void
Lat3DHEX1stO::PropagateG(const Vector Gi, const Vector G, Vector Gout) const{
	...
	for (i=1; i<= N_comp_ranges; i++){
		...
		addTimes(gs+jy,   gs_1,       g+jy ,  M-jy);
        	addTimes(gs+jy,   gs_1+jx,    g+jy,   M-jy-jx);
...
void
Lat3DHEX1stO::PropagateG(const Vector Gi, const Vector G, Vector Gout,const double f) const{
...
void
Lat3DHEX1stO::PropagateG(Matrix Gi, const Vector G, const int s) const {
	int i,jx,jy,M,Mprev=0;
	double *gs   = &Gi[1][s];
...
void
Lat3DHEX1stO::PropagateG(Matrix Gi, const Vector G, const int s,const double f) const {
	const double f1 = exp(f);
	const double f_1 = exp(-f);
	double fnorm = 1.0/(4.0 + 4.0*exp(f) + 4.0*exp(-f));
	int i,jx,jy,M,Mprev=0;
...
void
Lat3DHEX1stO::PropagateG(Vector Gi, const Vector G) const {
	int i,jx,jy,M,Mprev=0;
	double *gs   = &Gx[1];
	double *gs_1 = &Gi[1];
	const double *g = &G[1];
...
void
Lat3DHEX1stO::PropagateG(Vector Gi, const Vector G, const double f) const {
	const double f1 = exp(f);
	const double f_1 = exp(-f);
	double fnorm = 1.0/(4.0 + 4.0*exp(f) + 4.0*exp(-f));
	int i,jx,jy,M,Mprev=0;
...
void
Lat3DHEX1stO::Init2G(Vector Gi1, Vector Gi2, const Vector G, const LatticeRange* LatRange) const {
	int i,z,M,Mprev=0;
	for (i=1; i<=N_comp_ranges; i++) {
		M=CR_Q[i]->Get_M();
		if (i>1) Mprev +=CR_Q[i-1]->Get_M();
...
void
Lat3DHEX1stO::Propagate2G(Matrix Gi1, Matrix Gi2, const Vector G, const int s,const LatticeRange* LatRange) const {
	Message(fatal,"Propagate2G not implemented in 3d");
}
void
Lat3DHEX1stO::Propagate2G(Matrix Gi1, Matrix Gi2, const Vector G, const int s,const LatticeRange* LatRange,const double f) const {
	Message(fatal,MyInput,"Force ensemble not implemented in Lat3DHEX1stO");
}

void
Lat3DHEX1stO::Propagate2G(Vector Gi1, Vector Gi2, const Vector G, const LatticeRange* LatRange) const {
	Message(fatal,"Propagate2G not implemented in 3d");
}

void
Lat3DHEX1stO::Propagate2G(Vector Gi1, Vector Gi2, const Vector G, const LatticeRange* LatRange, const double f) const {
	Message(fatal,MyInput,"Force ensemble not implemented in Lat3DHEX1stO");
}

void
Lat3DHEX1stO::ConnectG(const Vector GiA, const Matrix GiB, const int s, Vector out) const {
...
void
Lat3DHEX1stO::ConnectG(const Vector GiA, const Vector GiB, Vector out) const {
	const double *p_giA = &GiA[1];
...
void
Lat3DHEX1stO::Connect2G(const Vector GiA1, const Matrix GiB1, const int s1, const Vector GiA2,
...
void
Lat3DHEX1stO::Connect2G(const Vector GiA1, const Vector GiB1, const Vector GiA2,  const Vector GiB2, Vector Out) const {
	const double *giA1 = &GiA1[1];
...
void
Lat3DHEX1stO::CorrectDoubleCountG(Vector in, const Vector G) const {
	double *p_in = &in[1];
	const double *p_G = &G[1];
	div(p_in,p_G,GetTotalNumLayers());
}
double
Lat3DHEX1stO::ComputeLnGN(Vector Gi) const {
	double *pGi = &Gi[1];
...
void
Lat3DHEX1stO::NormPhiFree(Vector phi, const double C) const {
	//Message(debug,"NormPhiFree(Vector phi, const double C)");
	norm(phi,C,GetTotalNumLayers());
}
void
Lat3DHEX1stO::NormPhiRestr(Vector phi, const Vector Gi, double C) const {
	//Message(debug,"NormPhiRestr(Vector phi)");
	C /= exp(ComputeLnGN(Gi));
	norm(phi,C,GetTotalNumLayers());
}
void
Lat3DHEX1stO::UpdateBoundaries(Vector A) const {
	//Message(debug,"UpdateBoundaries(Vector A)");
	double *pA=&A[1];
	SetBoundaries(pA);
}
void
Lat3DHEX1stO::UpdateBoundaries(Matrix A, const int s) const {
	double *pA=&A[1][s];
	SetBoundaries(pA);
}
