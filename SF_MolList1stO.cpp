...
SF_MolList1stO::SF_MolList1stO(SF_SegmentList* SegQ_,
							   Lat1stO* Lat_,
							   Input* MyInput_)
	...
	for (i=1; i<=numMolecules; i++) {
		...
		ring=MyInput->GetBoolean("mol",name,"ring",false);
		if (ring) {
			if (MolType != homopolymer) Message(fatal,"Only homopolymer rings are implemented (under construction)...");
		}
		delete Chain;
		switch (MolType) {
			case monomer:
				MoleculeQ[i] = new SF_Monomer1stO(name,SegQ,Lat,MyInput);
				break;
			case homopolymer:
				if (ring) MoleculeQ[i] = new SF_HomoRing1stO(name,SegQ,Lat,MyInput);
				else MoleculeQ[i] = new SF_Homopol1stO(name,SegQ,Lat,MyInput);
				break;
			case copolymer:
				MoleculeQ[i] = new SF_Copol1stO(name,SegQ,Lat,MyInput);
				break;
//			case polydisperse:
//				MoleculeQ[i] = new SF_Polydisp1stO(name,SegQ,Lat,MyInput);
//				break;
			case dendrimer:
				MoleculeQ[i] = new SF_Dend1stO(name,SegQ,Lat,MyInput);
				break;
			case asymmetric_dendrimer:
				MoleculeQ[i] = new SF_AsymDend1stO(name,SegQ,Lat,MyInput);
				break;
			case comb:
				MoleculeQ[i] = new SF_Comb1stO(name,SegQ,Lat,MyInput);
				break;
			case branched:
				MoleculeQ[i] = new SF_Branched1stO(name,SegQ,Lat,MyInput);
				break;
			default:
				Message(fatal,"Programming error in 'SF_MolList1stO::SF_MolList1stO'");
				break;
		}
	}
...
//			"Note: this error message is generated in SF_MolList1stO constructor");
//	}
	double maxTheta = 0;
...
void
SF_MolList1stO::GetOutput(Output* Out) const {
	SF_Molecule* Mol;
	if (SegQ->ReactionsPresent()) {
		...
		for (int i=1; i<=numMolecules; i++) {
			...
			if (Mol->GetMoleculeType()==asymmetric_dendrimer){
				int NG;
				int M=Lat->GetTotalNumLayers();
				Text valuetext=Blanks(100);
				Vector phiout1; phiout1.Dim(1,M);
				Vector phiout2; phiout2.Dim(1,M);
				Mol = MoleculeQ[i];
				Vector phix = Mol->GetShort();
				for (int z=1; z<=M; z++) phiout1[z]=phix[z];
				Out->PutProfile("mol",Mol->GetName(),"phi_shortest",phiout1);
				for (int z=1; z<=M; z++) phiout2[z]=phix[M+z];
				Out->PutProfile("mol",Mol->GetName(),"phi_longest",phiout2);
				Vector phiy = Mol->GetLong();
				NG=phiy.Length()/M-1;
				for (int p=0; p<=NG; p++){
					Vector phiout; phiout.Dim(1,M);
					for(int z=1; z<=M; z++)	phiout[z]=phiy[p*M+z];
					valuetext.Putint(p); valuetext=valuetext.Frontstrip();
					Out->PutProfile("mol",Mol->GetName(),"phi_"+valuetext,phiout);
				}
			} else if (Mol->GetMoleculeType()==comb){
				Vector phiout1=Mol->GetLong();
				Vector phiout2=Mol->GetShort();
				Out->PutProfile("mol",Mol->GetName(),"phi_backbone",phiout1);
				Out->PutProfile("mol",Mol->GetName(),"phi_teeth",phiout2);
			}
		}
	}
}
...
void
SF_MolList1stO::ComputePhi(double phiBulkSolv) {
	SF_Molecule* Mol;
	Mol = GetSolvent();
...
void
SF_MolList1stO::ComputePhi(double phiBulkSolv, double phiBulkNeutralizer) {
	SF_Molecule* Mol;
	Mol = GetSolvent();
    Mol->SetPhiBulk(phiBulkSolv);
	Mol = GetNeutralizer();
...
Boolean
SF_MolList1stO::CheckMolecules(double accuracy) const {
	SF_Molecule* Mol;
	SF_MolStructure* Chain;
...
double
SF_MolList1stO::GetChemicalPotential(int molNum, DensityPart densPart, int state) const {
	double value = 0;
	int i,j;
	SF_Molecule* Mol;
	SF_Molecule* Mol2;
	...
	if (N > 1 && state != 1) {
		Message(fatal,"programming error in call to SF_MolList1stO::GetChemicalPotential");
	}
	if (Mol->GetFreedom() == secondGeneration) {
...
double
SF_MolList1stO::GetChemicalPotential(const SF_Molecule* Mol, DensityPart densPart, int state) const {
	for (int i=1; i<=numMolecules; i++) {
		if (MoleculeQ[i] == Mol) return GetChemicalPotential(i,densPart,state);
	}
	Message(fatal,"error in call to SF_MolList1stO::GetChemicalPotential"
		"(const SF_Molecule* Mol)");
	return 0; // never get here
}
double
SF_MolList1stO::GetNumMolTimesChemPot(int molNum, DensityPart densPart) const {
	double value = 0;
	int i,j;
	SF_Molecule* Mol;
	SF_Molecule* Mol2;
...
double
SF_MolList1stO::GetNumMolTimesChemPot(const SF_Molecule* Mol, DensityPart densPart) const {
	for (int i=1; i<=numMolecules; i++) {
		if (MoleculeQ[i] == Mol) return GetNumMolTimesChemPot(i,densPart);
	}
	Message(fatal,"error in call to SF_MolList1stO::GetNumMolTimesChemPot"
		"(const SF_Molecule* Mol)");
	return 0; // never get here
}
double
SF_MolList1stO::GetChemicalPotentialLayer(int molNum, int z) const {
	double value = 0;
	int i,j;
	SF_Molecule* Mol;
	SF_Molecule* Mol2;
...
double
SF_MolList1stO::GetChemicalPotentialLayer(const SF_Molecule* Mol, int z) const {
	for (int i=1; i<=numMolecules; i++) {
...
double
SF_MolList1stO::GetFreeEnergy() const {
	double value = 0;
	int M = Lat->GetTotalNumLayers();
...
double
SF_MolList1stO::GetGrandPotential() const {
...
double
SF_MolList1stO::GetGrandPotential2() const {
...
double
SF_MolList1stO::GetGrandPotentialExcess() const {
	if (Lat->GetNumGradients()==2) {
		...
		if (Lat->GetVolume()!=n_layers_x*n_layers_y) {
			double FFV=0;
			double GP=GetGrandPotential();
			Vector w = GetGrandPotentialProfile();
			for (int y=1; y<=n_layers_y; y++) {
				FFV +=w[jx*n_layers_x+y];
			}
			return GP-FFV*PI*n_layers_x*n_layers_x;
		}
	}
	return 0;
}
...
double
SF_MolList1stO::GetSoumiFreeEnergy() const	{
	...
	for	(int i=1; i<=numMolecules; i++)	{
		...
		if (Mol->GetFreedom() == secondGeneration) {
			...
			if (Mol->GetIncludeTP()) value += thetaExcess*GetChemicalPotential(Mol)/Mol->GetChainLength();
		}
	}
	return value;
}
...
double
SF_MolList1stO::GetExcessFreeEnergy() const	{
	Vector Fexc	= GetGrandPotentialProfile();
	Lat->MultiplyWithLatticeSites(Fexc);
	int	M =	Lat->GetTotalNumLayers();
	double value = 0;
...
double
SF_MolList1stO::GetEntropy(SF_Molecule* Mol2) const {
	double value = 0;
	int M = Lat->GetTotalNumLayers();
...
double
SF_MolList1stO::GetContactInteractions() const {
	double value = 0;
	int M = Lat->GetTotalNumLayers();
...
double
SF_MolList1stO::GetInternalFreeEnergy() const {
	if (!SegQ->ReactionsPresent()) {
		return 0;
	}
	double value = 0;
...
double
SF_MolList1stO::GetElectricInteractions() const {
	if (!SegQ->Charged()) {
		return 0;
	}
	double value = 0;
...
Vector
SF_MolList1stO::GetFreeEnergyProfile() const {
	int M = Lat->GetTotalNumLayers();
...
Vector SF_MolList1stO::GetEntropyProfile(SF_Molecule* Mol2) const {
	SF_Molecule* Mol;
	double N;
	int i,j,z=0;
	int M = Lat->GetTotalNumLayers();
	...
	for (i=1; i<=numMolecules; i++) {
		...
		if (Mol->GetFreedom() == secondGeneration) {
			if (Mol->GetPhiBulk() > 0) {
				...
				for (z=1; z<=M; z++)
				entropy[z] -= (phi[z]/N)*(log(N) + Mol->GetLnNormFree());
			}
			...
			if (Chain->GetNumDiffSegments() > 1) {
				Message(fatal, "programming error in SF_MolList1stO::GetFreeEnergyProfileOld()");
			}
			for (j=1; j<=Chain->GetNumDiffSegments(); j++) {
				...
				for (z=1; z<=M; z++)
				entropy[z] -= (phi[z]/N)*log(N*norm[(z-1)/Lat->GetNumLayers(1)+1]);
			}
		} else if (Mol->GetFreedom() == fixedTheta || Mol->GetFreedom() == rangeRestricted) {
			phi = Mol->GetPhi(total);
			for (z=1; z<=M; z++)
			entropy[z] -= (phi[z]/N)*(log(N) + Mol->GetLnNormRestricted());
		} else {
			phi = Mol->GetPhi(total);
			for (z=1; z<=M; z++)
			entropy[z] -= (phi[z]/N)*(log(N) + Mol->GetLnNormFree());
		...
		if (Mol->GetMolStructure()->SomeSegmentsGrafted()) {
			...
			for (z=1; z<=M; z++)
			entropy[z] += (phi[z]/N)*log(phiGrafted);
		}
	}
...
Vector
SF_MolList1stO::GetContactInteractionsProfile() const {
	SF_Molecule* Mol;
	int i,z;
	int M = Lat->GetTotalNumLayers();
...
Vector
SF_MolList1stO::GetElectricInteractionsProfile() const {
	int M = Lat->GetTotalNumLayers();
...
Vector
SF_MolList1stO::GetInternalFreeEnergyProfile() const {
	int M = Lat->GetTotalNumLayers();
...
Vector
SF_MolList1stO::GetGrandPotentialProfile() const {
...
Vector
SF_MolList1stO::GetGrandPotentialProfile2() const {
	SF_Molecule* Mol;
	int M = Lat->GetTotalNumLayers();
	...
	for (int i=1; i<=numMolecules; i++) {
		...
		if (Mol->GetFreedom() == secondGeneration) {
			...
			Message(implementation,"SF_MolList1stO::GetPressureProfile for thirdgeneration");
		} else {
...
Vector
SF_MolList1stO::GetExcessFreeEnergyProfile() const {
	Vector Fexc;
	int z;
	if (SegQ->ReactionsPresent()) {
